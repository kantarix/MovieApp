...
import com.flethy.androidacademy.data.remote.retrofit.response.ImageResponse
...
class ImageUrlAppender(private val api: MovieApiService) {
    ...
    suspend fun getMoviePosterUrl(moviePosterPath: String?): String? {
        loadConfiguration()
        return buildUrl(baseUrl, posterSize, moviePosterPath)
    }

    suspend fun getDetailPosterUrl(backdropPath: String?): String? {
        loadConfiguration()
    ...
    suspend fun loadConfiguration() {
        ...
        backdropSize = imageResponse?.posterSizes?.find { size -> size == IDENTIFIER_LARGE_SIZE } ?: DEFAULT_SIZE
